...
/**
 * @component PixelFeathers
 * @description 画面上を漂うピクセルアートの羽根と、クリック時の音・パーティクル弾け演出を制御するインタラクティブ背景コンポーネントです。
 * 
 * [What]
 * 画面の左右をメインに30個のピクセル羽根を不均等に配置して浮遊させ、マウスホバーで静止、クリック・タップで Web Audio API による音階演奏とともに美しいピクセルパーティクルを飛散させつつ、別の位置に再出現させるライフサイクルを構築します。
 * 
 * [Why - 設計とインタラクション選定の理由]
 * 1. 静的になりがちな就職活動用のポートフォリオに対して、触って直感的に楽しいゲーム風の「インタラクティブ性」と「情緒的価値」を盛り込み、第一印象を強く印象づけるため。
 * 2. 複数の羽根がふわふわと動き回る際、手前にある自己紹介やプロジェクトのカードブロックのクリック（テキスト選択、ボタン操作）をブロックしないよう、親要素を `pointer-events-none`、クリック対象（羽根自体）のみを `pointer-events-auto` にし、さらにCSSの当たり判定領域を広げることで、デザイン性の高さとユーザビリティ（操作の邪魔にならないこと）を両立させるため。
 * 3. requestAnimationFrame による一元的な座標計算を採用し、CSSアニメーションでは困難な「ホバー時にその場に完全静止」「任意の座標から重力に従ったパーティクル飛散」「一定総数の維持」といった動的物理表現を軽量かつ滑らかに描画するため。
 */
